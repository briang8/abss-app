...
class WeatherService {
  ...
  static const _cityCoords = {
    'kigali_rw':    (-1.9441, 30.0619),
    'nairobi_ke':   (-1.2921, 36.8219),
    'addis_et':     (9.0320,  38.7469),
    'kampala_ug':   (0.3163,  32.5822),
    'dar_tz':       (-6.7924, 39.2083),
    'bujumbura_bi': (-3.3814, 29.3613),
  };
  ...
  static ForecastModel _parse(Map<String, dynamic> data, String locationId, String locationName) {
    final current = data['current_weather'] as Map<String, dynamic>;
    final hourly  = data['hourly']          as Map<String, dynamic>;
    final daily   = data['daily']           as Map<String, dynamic>;

    final currentTemp     = (current['temperature'] as num).toDouble();
    final currentWindKph  = (current['windspeed']   as num).toDouble();
    final currentWmoCode  = current['weathercode']  as int;
    final currentCondition = _wmoToCondition(currentWmoCode);

    // Parse hourly (next 24 hours)
    final hourlyTimes  = (hourly['time']                         as List).cast<String>();
    final hourlyTemps  = (hourly['temperature_2m']               as List).map((e) => (e as num).toDouble()).toList();
    final hourlyRain   = (hourly['precipitation_probability']    as List).map((e) => (e as num).toDouble()).toList();
    final hourlyWind   = (hourly['windspeed_10m']                as List).map((e) => (e as num).toDouble()).toList();
    ...
    for (int i = 0; i < hourlyTimes.length; i++) {
      ...
      if (t.isAfter(now) || t.isAtSameMomentAs(now)) { startIdx = i; break; }
    }
    ...
    final hourlyForecasts = List.generate(
      ...
      (i) {
        ...
        return HourlyForecast(
          ...
          tempC: hourlyTemps[idx],
          rainProbability: hourlyRain[idx],
          windKph: hourlyWind[idx],
    ...
    final dailyMax     = (daily['temperature_2m_max']            as List).map((e) => (e as num).toDouble()).toList();
    final dailyMin     = (daily['temperature_2m_min']            as List).map((e) => (e as num).toDouble()).toList();
    final dailyRain    = (daily['precipitation_probability_max'] as List).map((e) => (e as num).toDouble()).toList();
    final dailyMm      = (daily['precipitation_sum']             as List).map((e) => (e as num).toDouble()).toList();
    ...
    final dailyForecasts = List.generate(
      ...
      (i) => DailyForecast(
        ...
        condition: _wmoToCondition(dailyCodes[i]),
      ),
    );

    // Estimate humidity from rain probability (Open-Meteo free tier doesn't include relative humidity in this endpoint)
    final estimatedHumidity = (hourlyRain.isNotEmpty ? hourlyRain[startIdx] : 50.0) * 0.7 + 40;

    return ForecastModel(
      locationId: locationId,
      locationName: locationName,
      currentTemp: currentTemp,
      feelsLike: currentTemp - (currentWindKph * 0.05).clamp(0, 5),
      humidity: estimatedHumidity.clamp(30, 98),
      windKph: currentWindKph,
      rainProbability: hourlyForecasts.isNotEmpty ? hourlyForecasts.first.rainProbability : 0,
  ...
  static String _wmoToCondition(int code) {
    ...
    if (code <= 77) return 'rainy';  // snow (Africa — rare but possible at altitude)
