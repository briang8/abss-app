...
class FirestoreService {
  ...
  //  Users 
  static Future<void> createOrUpdateUser(String uid, Map<String, dynamic> data) async {
  ...
  static Future<void> updateUserSettings(String uid, Map<String, dynamic> settings) async {
    await _db.collection('users').doc(uid).collection('settings').doc('prefs').set(settings, SetOptions(merge: true));
  }

  static Future<void> updateFcmToken(String uid, String token, String platform) async {
    await _db.collection('users').doc(uid).collection('devices').doc(token).set({
      'fcm_token': token, 'platform': platform, 'last_seen': FieldValue.serverTimestamp(),
    });
  }
  ...
  static Future<void> markUserVerified(String uid) async {
    await _db.collection('users').doc(uid).update({
      'is_verified': true, 'verified_at': FieldValue.serverTimestamp(),
    });
  }

  //  Locations 
  static Future<DocumentSnapshot?> getLocation(String locationId) async {
    try { return await _db.collection('locations').doc(locationId).get(); }
    catch (_) { return null; }
  }

  //  Forecasts 
  static Stream<QuerySnapshot> forecastsStream(String locationId) {
    return _db.collection('forecasts')
  ...
  static Future<void> saveForecast(ForecastModel forecast) async {
    await _db.collection('forecasts').add({
      ...
      'valid_to':      Timestamp.fromDate(forecast.generatedAt.add(const Duration(days: 7))),
      'temperature_min': forecast.daily.isNotEmpty ? forecast.daily.first.tempMin : 0,
      'temperature_max': forecast.daily.isNotEmpty ? forecast.daily.first.tempMax : 0,
      'rainfall_mm':   forecast.daily.isNotEmpty ? forecast.daily.first.rainfallMm : 0,
  ...
  static Future<void> acknowledgeAlert(String uid, String alertId) async {
    await _db.collection('users').doc(uid).collection('acknowledged_alerts').doc(alertId)
        .set({'acknowledged_at': FieldValue.serverTimestamp()});
  }

  //  Content Localization 
  static Future<Map<String, String>> getLocalizedContent(String key, String language) async {
    try {
      final snap = await _db.collection('content_localization')
...
class PhoneVerificationService {
  ...
  static Future<PhoneVerificationResult> sendCode({
    ...
    // Call a Firebase Cloud Function that:
    //   1. Generates a 6-digit OTP
    //   2. Stores it in Firestore with a 10-minute expiry
    //   3. Calls MTN SMS API v3: POST https://api.mtn.com/v3/sms/messages/sendSMS
    //      { senderAddress:"ABSS", receiverAddress:[phone], message:"Your ABSS code: XXXXXX" }
  ...
  static PhoneVerificationResult verifyCode(String entered, String expected) {
    if (entered.trim() == expected.trim()) return const PhoneVerificationResult.verified();
    return const PhoneVerificationResult.failure('Incorrect code. Please try again.');
  }

  static String buildWelcomeMessage(String langCode) {
    const msgs = {
      'en': 'Welcome to ABSS! Your number is verified. You will receive critical hazard alerts for your area.',
      'sw': 'Karibu ABSS! Nambari yako imethibitishwa. Utapokea arifa za hatari muhimu kwa eneo lako.',
      'rw': 'Murakaza neza ABSS! Numero yawe yemejwe. Uzahabwa inzitizi z\'ibyago mu karere kawe.',
      'am': 'ABSS እንኳን ደህና መጡ! ቁጥርዎ ተረጋግጧል። ለአካባቢዎ ወሳኝ ማስጠንቀቂያዎች ይደርሳሉ።',
      'fr': 'Bienvenue sur ABSS ! Votre numéro est vérifié. Vous recevrez des alertes de danger critique.',
    };
...
class PhoneVerificationResult {
  ...
  const PhoneVerificationResult.codeSent(this.code) : success = true, codeSentFlag = true, error = null;
  const PhoneVerificationResult.verified()           : success = true, codeSentFlag = false, code = null, error = null;
  const PhoneVerificationResult.failure(this.error)  : success = false, codeSentFlag = false, code = null;
}
