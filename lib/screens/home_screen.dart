            ...
            slivers: [
              ...
              SliverToBoxAdapter(
                child: activeAlerts.isEmpty
                    ...
                    : _ActiveAlertBanner(alert: activeAlerts.first),
              ),
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '$greeting${profile.name.isNotEmpty ? ", ${profile.name.split(' ').first}" : ""}',
            ...
            child: Row(
              children: [
                ...
                Text(
                  ...
                  style: AppText.caption(context).copyWith(fontSize: 12),
                ),
              ],
            ),
          ),
...
class _WeatherHero extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final current = forecast.hourly.isNotEmpty ? forecast.hourly.first : null;
    final today = forecast.daily.isNotEmpty ? forecast.daily.first : null;
    final currentTemp = current?.temperatureC ?? 0;
    final currentCondition = current?.condition ?? 'cloudy';
    final windKph = current?.windSpeedKph ?? 0;
    final rainProbability = current?.rainProbability ?? 0;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _conditionLabel(currentCondition),
                      style: AppText.body(context).copyWith(fontSize: 16),
                    ),
                    Text(
                      today == null
                          ? 'Updated now'
                          : 'Range ${today.tempMinC.round()}°C - ${today.tempMaxC.round()}°C',
                      style: AppText.caption(context),
                    ),
                  ],
                ),
              ),
              WeatherIcon(condition: currentCondition, size: 72),
            ],
          ),
          ...
          Row(
            children: [
              _StatPill(
                ...
                value: '${forecast.daily.length} days',
                label: 'Forecast',
              ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: AppText.h4(context).copyWith(fontSize: 16),
                    ),
                    Text(
                      '${h.rainProbability.round()}%',
...
class _HourlyStripSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: ShimmerBox(width: 120, height: 18, radius: 6),
      ),
      const SizedBox(height: 12),
      SizedBox(
        height: 108,
        child: ListView.separated(
          ...
          separatorBuilder: (_, _) => const SizedBox(width: 8),
          itemBuilder: (_, _) =>
              const ShimmerBox(width: 72, height: 108, radius: 14),
        ),
      ),
    ],
  );
