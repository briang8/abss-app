...
(String, int, List<String>, String) _rulesFor(String locId) =>
...
class _OnboardingState extends ConsumerState<OnboardingScreen>
  ...
  Widget _buildStep() {
    switch (_step) {
      case 0:
        return _WelcomeStep(l: _l, onStart: _next);
      case 1:
        return _LanguageStep(
          l: _l,
          selected: _langName,
          onSelect: (n, c) => setState(() {
            _langName = n;
            _langCode = c;
          }),
          onNext: _next,
      ...
      case 2:
        return _LocationStep(
          l: _l,
          selectedId: _locId,
          onSelect: (id, name, lat, lng) => setState(() {
            _locId = id;
            _locName = name;
            _locLat = lat;
            _locLng = lng;
      ...
      case 3:
        return _UserTypeStep(
          l: _l,
          selected: _regType,
          onSelect: (t) => setState(() => _regType = t),
          onNext: _next,
        );
      default:
        return _regType == _RegType.offline
            ? _OfflineSetupStep(
                ...
                onToggle: (t) => setState(() {
                  _alertTypes.contains(t)
                      ? _alertTypes.remove(t)
                      : _alertTypes.add(t);
                }),
                onDone: _finish,
              )
      ...
      child: Column(
        children: [
          ...
          _FeatureRow(
            ...
            text: 'Early disaster alerts via push & SMS',
          ),
          const SizedBox(height: 12),
          _FeatureRow(
            icon: Icons.cloud_queue_outlined,
            color: AppColors.info,
            text: 'Offline-first weather forecasts',
          ),
          const SizedBox(height: 12),
          _FeatureRow(
            icon: Icons.language_outlined,
            color: AppColors.primary,
            text: 'Available in 5 languages',
          ),
          const Spacer(flex: 2),
          AbssButton(
            label: l.getStarted,
...
class _LocationStep extends ConsumerStatefulWidget {
  ...
  final void Function(String id, String name, double? lat, double? lng)
  onSelect;
  final VoidCallback onNext;
  const _LocationStep({
    required this.l,
  ...
  Future<void> _detectLocation() async {
    ...
    try {
      ...
      if (perm == LocationPermission.denied ||
        ...
        setState(() {
          _detecting = false;
          _detectError = 'Location permission denied. Please choose manually.';
        });
        return;
      }
      ...
      setState(() {
        _detecting = false;
        _detectError = 'Could not detect location. Please choose manually.';
      });
    }
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _Tile(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    widget.l.detectLocation,
                    style: AppText.bodyMedium(context),
                  ),
                ),
                if (widget.selectedId == 'auto')
                  const Icon(
                    Icons.check_rounded,
                    color: AppColors.primary,
                    size: 20,
          ...
          AbssButton(
            label: widget.l.cont,
            onTap: widget.selectedId.isNotEmpty ? widget.onNext : null,
          ),
          const SizedBox(height: 16),
        ],
      ),
...
class _UserTypeStep extends StatelessWidget {
  ...
  final void Function(_RegType) onSelect;
  final VoidCallback onNext;
  const _UserTypeStep({
    required this.l,
    required this.selected,
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        _TypeCard(
          selected: selected == _RegType.online,
          onTap: () => onSelect(_RegType.online),
          icon: Icons.wifi_rounded,
          iconColor: AppColors.info,
          title: l.onlineMode,
          subtitle: l.onlineModeDesc,
          bullets: l.onlineBenefits,
        ),
        const SizedBox(height: 14),
        _TypeCard(
          selected: selected == _RegType.offline,
          onTap: () => onSelect(_RegType.offline),
          icon: Icons.sms_outlined,
          iconColor: AppColors.primary,
          title: l.offlineMode,
          subtitle: l.offlineModeDesc,
          bullets: l.offlineBenefits,
        ),
    ...
    child: AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            subtitle,
            style: AppText.body(context).copyWith(fontSize: 13, height: 1.5),
          ),
          if (selected) ...[
            const SizedBox(height: 12),
            Container(
...
class _OnlineSetupStep extends StatefulWidget {
  ...
  @override
  State<_OnlineSetupStep> createState() => _OnlineSetupStepState();
}
      ...
      child: Column(
        ...
        children: [
          ...
          _InputCard(
            label: 'Phone number',
            note:
                '$dialCode · $digitLen digits, starts with ${firstDigits.join(' or ')}',
            error: _phoneError,
            child: Row(
              children: [
                Container(
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Number verified! You\'re all set.',
                      style: AppText.bodyMedium(
                        context,
...
class _OfflineSetupStep extends StatefulWidget {
  ...
  @override
  State<_OfflineSetupStep> createState() => _OfflineSetupStepState();
}
        ...
        children: [
          ...
          if (_verifStep == _VerifStep.verified) ...[
              ...
              children: _hazards.map((h) {
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: sel
                          ? h.$4.withValues(alpha: 0.1)
                          : AppColors.card(context),
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(
                        color: sel
                            ? h.$4.withValues(alpha: 0.5)
                            : AppColors.border(context),
                        width: sel ? 1.5 : 1,
                      ),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          h.$3,
                          size: 15,
                          color: sel ? h.$4 : AppColors.textMuted(context),
                        ),
                        ...
                        Text(
                          ...
                          style: AppText.bodyMedium(context).copyWith(
                            fontSize: 13,
                            color: sel ? h.$4 : AppColors.textPrimary(context),
                          ),
                        ),
                      ],
                    ),
            ...
            AbssButton(
              ...
              icon: Icons.arrow_forward_rounded,
              isLoading: _loading,
              onTap: (widget.alertTypes.isNotEmpty && !_loading)
                  ? _submit
                  : null,
            ),
          ],
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              style: AppText.caption(
                context,
              ).copyWith(fontSize: 11, color: AppColors.textSecondary(context)),
            ),
            const SizedBox(height: 6),
            child,
          ],
      ...
      if (error != null)
        Padding(
          ...
          child: Text(
            ...
            style: AppText.caption(
              context,
            ).copyWith(fontSize: 11, color: AppColors.critical),
          ),
        ),
    ],
  );
